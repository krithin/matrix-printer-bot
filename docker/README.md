...
The docker image will run matrix-printer-bot with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/matrix-printer-bot?sslmode=disable"
  ```
...
```
docker-compose up matrix-printer-bot
```
...
```
docker-compose up -d matrix-printer-bot
```

(Logs can later be accessed with the `docker logs` command).

This will use the `latest` tag from
[Docker Hub](https://hub.docker.com/somebody/matrix-printer-bot).
...
```
docker-compose pull matrix-printer-bot
```
...
[matrix-printer-bot.service](matrix-printer-bot.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/matrix-printer-bot.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start matrix-printer-bot
```

and stop it with:

```
systemctl stop matrix-printer-bot
```

To run the bot on system startup:

```
systemctl enable matrix-printer-bot
```
...
```
docker build -t somebody/matrix-printer-bot:latest -f docker/Dockerfile .
```
